...
const { Event, Event_Image, User, Venue, Attendance } = require("../../db/models");
const { validateEvent, validateAttendanceUpdate, validateQueryParams } = require("../../utils/validation");
...
router.post("/:eventId/attendance", requireAuth, async (req, res, next) => {
	try {
		...
		if (isMember) {
			if (isMember["Memberships.status"] === "member" || isMember["Memberships.status"] === "co-host") {
				userMembership = true;
			}
		}
